...
| system | `system.get_load_average` | YES: `/proc/loadavg` | YES | L0 | CPU investigation | DISCOVERED |
| system | `system.get_kernel_info` | YES: uname/os-release | YES | L0 | Target probe support | DISCOVERED |
| system | `system.get_mounts` | YES: proc mounts | YES | L0 | Disk investigation | DISCOVERED |
| system | `system.get_uptime` | YES: proc uptime | YES | L0 | Overview | DISCOVERED |
| process | `process.list_top` | YES: proc PID stat/status | YES | L0 | CPU investigation | DISCOVERED |
| process | `process.search` | YES: redacted proc cmdline | YES | L0 | Process investigation | CALLED |
| process | `process.get_details` | YES: PID/start ticks identity | YES | L0 | Port/CPU investigation | DISCOVERED |
| process | `process.get_resource_usage` | YES: CPU ticks/RSS | YES | L0 | CPU investigation | DISCOVERED |
| process | `process.find_by_port` | YES: socket inode ↔ FD | YES | L0 | Port-conflict investigation | CALLED |
| network | `network.list_listeners` | YES: proc net tables | YES | L0 | Port-conflict investigation | DISCOVERED |
| network | `network.list_connections` | YES: proc net tables | YES | L0 | Network investigation | DISCOVERED |
| network | `network.check_port` | YES: socket + visible owner | YES | L0 | Port-conflict investigation | CALLED |
| network | `network.get_interfaces` | YES: Go net + proc net/dev | YES | L0 | Overview | DISCOVERED |
| network | `network.get_interface_stats` | YES: proc net/dev | YES | L0 | Network investigation | DISCOVERED |
| journal | `journal.get_recent` | YES: fixed bounded JSON command | YES | L0 | General investigation | DISCOVERED |
| journal | `journal.query_unit` | YES: validated unit | YES | L0 | Port-conflict/service | CALLED |
| journal | `journal.search_errors` | YES: bounded literal filter | YES | L0 | RCA evidence | DISCOVERED |
| journal | `journal.get_priority_events` | YES: priority 0-7 | YES | L0 | RCA evidence | DISCOVERED |
| service | `service.get_status` | YES: fixed show properties | YES | L0 | Port-conflict/service | CALLED |
| service | `service.list_failed` | YES: fixed list-units | YES | L0 | Overview/investigation | DISCOVERED |
| service | `service.get_dependencies` | YES: fixed properties | YES | L0 | RCA evidence | DISCOVERED |
| service | `service.get_restart_count` | YES: NRestarts/MainPID | YES | L0 | RCA evidence | DISCOVERED |
...
| diagnostic | `diagnostic.build_snapshot` | YES: bounded multi-source snapshot | YES | L0 | Evidence capture | DISCOVERED |
...
| file | `file.list_directory` | YES: bounded directory metadata | YES | L0 | File investigation | DISCOVERED |
| file | `file.sha256` | YES: allowlisted, 16 MiB hard bound | YES | L0 | Target/config identity | DISCOVERED |
| file | `file.find_large` | YES: depth/result bounded, no symlink follow | YES | L0 | Disk/file investigation | CALLED |
| config | `config.list_roots` | YES: configured allowlist only | YES | L0 | Configuration visibility | DISCOVERED |
| config | `config.get_metadata` | YES: metadata, no content | YES | L0 | Configuration investigation | DISCOVERED |
| config | `config.snapshot` | YES: per-file/total bounded hashes | YES | L0 | Configuration change | DISCOVERED |
| config | `config.diff_snapshot` | YES: validated baseline vs current hashes | YES | L0 | Configuration change | DISCOVERED |

All 39 tools use typed official SDK schemas and are present in the local security catalog. Protocol evidence includes per-server in-memory initialize/list/call and an official Kylin V11/loong64 Registry run where eight installed stdio Servers initialized, responded to ping and exposed 39 fingerprints. Audited target workflows made structured calls to the 15 tools marked `CALLED`; the remaining 24 are conservatively marked only `DISCOVERED`. Registry tests also cover disable/re-enable, rediscovery, stable fingerprints and detected tool-list changes.
