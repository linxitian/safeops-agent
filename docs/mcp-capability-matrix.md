...
All 39 tools use typed official SDK schemas and are present in the local security catalog. Protocol evidence includes per-server in-memory initialize/list/call and official Kylin V11/loong64 Registry runs where eight installed stdio Servers initialized, responded to ping and exposed 39 fingerprints. Candidate runtime `b5383e9` (intermediate PR #21 commit; squash-merged as `816f8cf`) added a unique, individually bounded target call plan; checksum-verified report `target_ae6d4bbeb9ae7b8e5764` records successful structured calls for all 39 tools as the non-root `safeops` identity. Registry tests also cover disable/re-enable, rediscovery, stable fingerprints and detected tool-list changes.

The Registry now records the actual implementation name, implementation version and negotiated protocol version from the official SDK initialize result. A single non-overlapping loop checks only enabled servers at a configurable interval and per-server timeout; disconnected servers are rediscovered. Manifest dependencies are never executed: absolute paths use filesystem metadata only and bare command names use executable lookup plus metadata only. The server-list API and Tool view expose dependency results and the latest 32 bounded health and discovery attempts, including failed attempts followed by healthy protocol recovery. This new scope is `TESTED` on Ubuntu and awaits exact installed Kylin verification; it does not change the 8-server/39-tool count or any risk level.

The same typed Platform/SafeFS values now feed seven collector-independent Observation sources with per-collector timeout, partial-error and output budgets. Installed candidate runtime `053fc2c` (intermediate PR #27 commit; squash-merged as `4861dcf`) executed all seven Collectors and the Prometheus/OpenTelemetry adapter models as non-root `safeops`; target report `target_df68d477155fd8d55d75` persisted only counts. This does not add tools or change the count: the MCP matrix remains exactly 8 Servers and 39 L0 tools, and no external telemetry platform deployment is claimed.
...
Write names in local policy are not MCP Tools and are never exposed to the model as arbitrary execution capabilities. The general planner may now request only local `service.restart` or `process.terminate` capabilities after explicit operator action intent and exact structured identity evidence from a related successful MCP result; error text, unrelated Tool output and PID/start-ticks split across objects fail closed. Those requests still become signed, target-snapshotted, approval-bound envelopes and never become MCP calls. `safeops-privexec` has fixed dry-run handlers and an explicit `lab` mode with allowlisted file create, reversible delete-by-quarantine, quarantine/restore, service restart and SIGTERM process termination handlers. Earlier target port/CPU/disk/file flows verified the used process termination, service restart, quarantine and restore handlers across the signed-envelope, approval, scope and target-revalidation boundary; the new general-planner request path is `TESTED` on Ubuntu and awaits separate target evidence. Create/delete retain Ubuntu test evidence only, permanent purge has no handler, and no write MCP Tool is callable.

The file MCP default roots are now `/var/log` and `/var/lib/safeops/lab`; this changes neither the 8-server/39-tool count nor any write scope. The Web path browser has a separate default read-only `/` metadata view, while executor-managed writes retain `/home` and the existing Lab root. Browser traversal and child-directory creation are confined by directory-root handles so symbolic links cannot escape a configured root; this new UI/API scope is `TESTED`, not yet target-verified.
...
The installed candidate `2b26de4` (intermediate PR #23 commit; squash-merged as `7479752`) target-served Web console was also traversed with real Chrome. Its Tool view rendered 8/8 healthy Servers and 39 Tools, while all 39 browser responses across the eight-view audit returned HTTP 200. This corroborates the installed Registry projection but does not replace or alter the per-tool native `CALLED` evidence above. Candidate, PR-head, and merge identities are indexed in [`docs/evidence`](evidence/README.md).

The later exact merged `7479752` installation repeated the unique 39/39 native call plan before running all six benchmark suites as non-root `safeops`. The benchmark uses controlled fixtures, fake MCP observations for tool-selection scoring, DryRun executor handlers, and temporary scoped rollback files; it neither adds Tool calls to this matrix nor changes the fixed count of 8 Servers and 39 L0 Tools.
