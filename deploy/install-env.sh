...
safeops_install_version() {
  ...
  if [[ "$version_source" != /* || "$version_destination" != /* ]]; then
    echo "SafeOps version paths must be absolute" >&2
    return 1
  fi
  ...
  if [[ "${#version_lines[@]}" -ne 1 || ! "${version_lines[0]}" =~ ^[A-Za-z0-9][A-Za-z0-9._+-]{0,127}$ ]]; then
