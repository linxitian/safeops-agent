...
Write names in local policy are not MCP Tools and are never exposed to the model as arbitrary execution capabilities. `safeops-privexec` has fixed dry-run handlers and an explicit `lab` mode with allowlisted quarantine/restore, service restart and SIGTERM process termination handlers. Their signed-envelope, approval, scope and target-revalidation boundaries pass Ubuntu tests; none is target-tested, permanent purge has no handler, and no write MCP Tool is callable.
