...
| M4 General Agent Runtime | TARGET_VERIFIED | Credentialed compatible-provider runs on official Kylin completed real MCP reads; candidate `d0d6642` with DeepSeek V4 Flash completed scoped `/var/log` diagnosis in 6.1 seconds with correct numeric evidence and a `VALID` Trace |
...
| Credentialed compatible-provider runs | TARGET_VERIFIED; DeepSeek V4 Flash target task `task_b91a18bc41a6d713030be924` completed in 6.1 seconds with one scoped read, one exact evidence citation and a `VALID` Trace |
