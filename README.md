...
- 通用 Agent Runtime 已用真实兼容 Provider 在目标机验证；候选 `384f06c` 使用 DeepSeek V4 Flash 在 21.1 秒内完成真实 `/var/log` 调查，正确报告 13.86% 使用率、披露权限受限证据并精确引用两条 `VALID` Trace 证据。近期用户/助手消息和有序已选资源会以有界、脱敏结构进入追问规划，单次 Provider 调用受持久 Agent 截止时间约束。无 Provider 时仍可使用 CPU/内存确定性纵切片。
