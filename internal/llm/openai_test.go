...
func TestDecisionSystemPromptConstrainsAmbiguousFollowupsToSelectedResources(t *testing.T) {
	...
	if !strings.Contains(decisionSystemPrompt, "local_read_scope is an authoritative local-policy boundary") || !strings.Contains(decisionSystemPrompt, "never authorizes expanding to another root") || !strings.Contains(decisionSystemPrompt, "guard_feedback") {
